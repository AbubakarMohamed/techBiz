...
export default function ModalInlineCalendly({
  ...
  // Ensure widget reinitializes when modal opens (desktop only)
  useEffect(() => {
    if (isOpen && !isMobile) {
  ...
  // Redirect on mobile if trying to open modal
  useEffect(() => {
    if (isOpen && isMobile) {
      console.log("Mobile detected - opening Calendly in new tab");
      window.open(fullUrl, "_blank");
      onClose?.(); // Close modal trigger to avoid overlay staying open
    }
  }, [isOpen]);
