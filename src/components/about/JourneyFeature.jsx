...
const VerticalTimelineNode = React.memo(({ milestone, index, currentIndex, goToSlide }) => (
  <div className="relative flex items-center group">
    ...
    <div className="ml-6 flex-1">
      <div className={`transition-all duration-300 ${
        currentIndex === index ? 'opacity-100' : 'opacity-70 hover:opacity-90'
      }`}>
        <div className={`text-sm font-light mb-1 transition-colors duration-300 ${
          currentIndex === index
            ? 'text-blue-400'
            : 'text-gray-400'
        }`}>
...
const MinimalProgressDot = React.memo(({ index, currentIndex, goToSlide, milestone }) => (
  <button
    onClick={() => goToSlide(index)}
...
const JourneyCarousel = () => {
  ...
  const currentMilestone = useMemo(() => journeyMilestones[currentIndex], [currentIndex]);
  ...
  const handleKeyDown = useCallback((event) => {
    if (event.target.closest('[role="button"]') || event.target.closest('button')) {
      switch (event.key) {
        case 'ArrowUp':
        case 'ArrowLeft':
  ...
  const goToPrevious = useCallback(() => {
    setCurrentIndex(prev => (prev - 1 + journeyMilestones.length) % journeyMilestones.length);
  }, [journeyMilestones.length]);
  ...
  />
        ...
        <div className="mt-4 sm:mt-6">
          ...
          <div className="lg:col-span-1 hidden md:block">
            <div className="lg:sticky lg:top-8">
              ...
              {/* Vertical Timeline */}
              <div className="space-y-8" role="tablist" aria-label="Journey milestones">
        ...
        <div className="mt-6 lg:hidden flex justify-center">
          <div className="flex items-center gap-2" aria-label="Journey progress">
            {journeyMilestones.map((milestone, index) => (
              <button
                key={index}
                onClick={() => goToSlide(index)}
