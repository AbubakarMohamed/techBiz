...
export default function SimplifiedCertificates({ showContent, isMobile, centerContent }) {
  return (
    <motion.div
    ...
    >
      ...
      {certificates.map((cert) => (
        ...
        >
          {/* Certificate Icon */}
          <div className={`${isMobile ? 'w-8 h-8' : 'w-12 h-14 lg:w-16 lg:h-16'} relative ${isMobile ? '' : '-ml-1 md:-ml-2.5'} flex-shrink-0 transition-transform duration-300 ${isMobile ? '' : 'group-hover:scale-90'}`}>
            <Image
              src={cert.src}
              alt={cert.alt}
              fill
